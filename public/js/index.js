(function ($) {
  ...
  window.MyGlobalVariables.url = $.extend(true, window.MyGlobalVariables.url, {
    ...
    addressCompany: {
      get: '/ajax/user/address/company/get',
      add: '/ajax/user/address/company/add',
      edit: '/ajax/user/address/company/edit',
      remove: '/ajax/user/address/company/remove',
      all: '/ajax/user/address/company/all',
    },
    returnOrder: {
      items: '/ajax/user/return-order/add',
    },
    order: {
      pdfExport: '/ajax/user/order/export/pdf',
    },
  });
  ...
  $(function () {
    ...
    function reAssignThings() {
      ...
      $('.edit-company-element-item')
        .off('click' + variables.namespace)
        .on('click' + variables.namespace, function () {
          editAddressCompanyBtnClick(this);
        });
    }
    ...
    // ADD ADDRESS COMPANY
    //---------------------------------------------------------------
    shop.forms.submitForm('addAddressCompany', constraints.addAddressCompany, function (values) {
      // do ajax
      if (createLoader) {
        createLoader = false;
        loaderId = shop.showLoader();
      }
      shop.request(variables.url.addressCompany.add, 'post', function () {
        shop.hideLoader(loaderId);
        // clear element after success
    ...
    //---------------------------------------------------------------
    shop.forms.submitForm('editAddressCompany', constraints.editAddressCompany, function (values) {
      if (editAddrCompanyId) {
        // do ajax
        if (createLoader) {
          createLoader = false;
          loaderId = shop.showLoader();
        }
        shop.request(variables.url.addressCompany.edit + '/' + editAddrCompanyId, 'post', function () {
          shop.hideLoader(loaderId);
          // clear element after success
          $(variables.elements.editAddressCompany.form).get(0).reset();
    ...
    //---------------------------------------------------------------
    // DOWNLOAD USER'S ORDER FACTOR
    //---------------------------------------------------------------
    $('#excelPdfOrder').on('click', function () {
      var code = $(this).attr('data-export-code');
      if (code) {
        window.location = variables.url.order.pdfExport + '/' + code;
      }
    });
  });
})(jQuery);
